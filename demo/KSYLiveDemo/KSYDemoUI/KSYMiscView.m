...
- (id)init{
    self = [super init];
    _btn0  = [self addButton:@"Str screenshots are files"];//str截图为文件
    _btn1  = [self addButton:@"str screenshot for UIImage"];//str截图为UIImage
    _btn2  = [self addButton:@"filter screenshot"];//filter截图
    
    _btn3  = [self addButton:@"select Logo"];//选择Logo
    _btn4  = [self addButton:@"shoot Logo"];//拍摄Logo
    
    _lblRec       = [self addLable:@"bypass recording"];//旁路录制
    ...
    _lblScene      = [self addLable:@"live scene"];//直播场景
    _liveSceneSeg  = [self addSegCtrlWithItems:@[ @"default", @"show", @"game"]];//默认", @"秀场", @"游戏
    _lbrScene      = [self addLable:@"record the screen"];//录制场景
    _recSceneSeg  = [self addSegCtrlWithItems:@[ @"constant bitrate", @"constant quality"]];//恒定码率", @"恒定质量
    _lblPerf       = [self addLable:@"coding performance"];//编码性能
    _vEncPerfSeg   = [self addSegCtrlWithItems:@[ @"low power consumtion", @"balanced", @"high performance"]];//低功耗", @"均衡", @"高性能
    _autoReconnect = [self addSliderName:@"Auto number of reconnections" From:0.0 To:10 Init:3];//自动重连次数
    return self;
}
